...
def chunk_document(doc: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    """
    # Use the smarter approach: split on paragraphs and detect section headers
    text = doc["text"]
    source = doc["source"]
    title = doc["title"]
    ...
    for para in paragraphs:
        ...
        # Heuristic: detect section headers (short lines without terminal punctuation)
        is_section_header = (
            len(para) < 100 and 
...
def embed_texts(texts: list[str]) -> list[list[float]]:
    ...
    # Extract embeddings in the same order as inputs
...
def answer(question: str, k: int = 3) -> dict[str, Any]:
    ...
    context_parts = []
    for chunk in chunks:
        source_header = f"[Source: {chunk['title']} — {chunk['section']}]"
...
Please answer the question using ONLY the provided excerpts. Cite the source (story title + section) in your answer."""
